...
{
    [SyncVar(hook = nameof(OnChangeExperienceLength))]
    public float experienceLengthSeconds;
    [SyncVar(hook = nameof(OnChangeDeathLengthSeconds))]
    public float deathLengthSeconds;
    [SyncVar(hook = nameof(OnChangeFaderLevelsMax))]
    public float faderLevelsMax;
    [SyncVar(hook = nameof(OnChangeBugPushStrength))]
    public float bugPushStrength;
    [SyncVar(hook = nameof(OnChangeBugPullStrength))]
    public float bugPullStrength;

    [SyncVar(hook = nameof(OnChangeResetHeadset))]
    public bool resetHeadset = false;
    [SyncVar(hook = nameof(OnChangeResetHeadsetImmediate))]
    public bool resetHeadsetImmediate = false;
    [SyncVar(hook = nameof(OnChangePauseHeadset))]
    public bool pauseHeadset = false;

    [SyncVar(hook = nameof(OnChangeHeadsetText))]
    public string headsetText;
    [SyncVar(hook = nameof(OnChangeWhichHeadset))]
    public int whichHeadset;
    [SyncVar(hook = nameof(OnChangeFenceSize))]
    ...
    void Start()
    {
        ...
        {
            foreach (var item in hideIfClient)
            {
    ...
    public void OnChangeExperienceLength(float oldVal, float newVal)
    {
        experienceLengthSeconds = newVal;
        levelHandler.timeMax = newVal;
    }

    public void OnChangeDeathLengthSeconds(float oldVal, float newVal) { deathLengthSeconds = newVal; }
    public void OnChangeFaderLevelsMax(float oldVal, float newVal) { faderLevelsMax = newVal; }
    public void OnChangeBugPushStrength(float oldVal, float newVal) { bugPushStrength = newVal; }
    public void OnChangeBugPullStrength(float oldVal, float newVal) { bugPullStrength = newVal; }
    public void OnChangeResetHeadset(bool oldVal, bool newVal) { resetHeadset = newVal; }
    public void OnChangePauseHeadset(bool oldVal, bool newVal) { pauseHeadset = newVal; }
    public void OnChangeResetHeadsetImmediate(bool oldVal, bool newVal) { resetHeadsetImmediate = newVal; }
    public void OnChangeWhichHeadset(int oldVal, int newVal) { whichHeadset = newVal; }
    public void OnChangeFenceSize(float oldVal, float newVal) { fenceSize = newVal; }

    public void OnChangeHeadsetText(string oldVal, string newVal) { headsetText = newVal; }
    ...
    void Update()
    {
        ...
        if (Input.GetKeyUp(KeyCode.Alpha0)) { WhichHeadset = 0; }
        if (Input.GetKeyUp(KeyCode.Alpha1)) { WhichHeadset = 1; }
        if (Input.GetKeyUp(KeyCode.Alpha2)) { WhichHeadset = 2; }
        if (Input.GetKeyUp(KeyCode.Alpha3)) { WhichHeadset = 3; }
        if (Input.GetKeyUp(KeyCode.Alpha4)) { WhichHeadset = 4; }
        if (Input.GetKeyUp(KeyCode.Alpha5)) { WhichHeadset = 5; }
        if (Input.GetKeyUp(KeyCode.Alpha6)) { WhichHeadset = 6; }
        if (Input.GetKeyUp(KeyCode.Alpha7)) { WhichHeadset = 7; }
        if (Input.GetKeyUp(KeyCode.Alpha8)) { WhichHeadset = 8; }
        if (Input.GetKeyUp(KeyCode.Alpha9)) { WhichHeadset = -1; }
        ...
        foreach (var fade in fader)
        {
            fade.maxLevel = faderLevelsMax;
        }
